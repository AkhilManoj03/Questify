    ...
    return (
        <div className={`m-auto px-20 bg-black z-[9999] duration-500 text-[#9fd5e2] flex items-center justify-between fixed top-0 pt-5 left-0 w-screen h-20 ${isVisible ? "translate-y-0" : "-translate-y-full"}`}>
            ...
            <div className="space-x-8 hover:text-[#b5e0ec]">
              <a className="text-lg " href='/user'>
                  User
              </a>
              <Link className="text-lg cursor-pointer " href='/search'>
                  Search
              </Link>
              <Link className="text-lg cursor-pointer" href='/admin'>
                  Admin
              </Link>
              <button className="text-lg cursor-pointer" onClick={() => signOut()}>
                  Sign out
              </button>
            </div>
