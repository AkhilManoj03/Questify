...
export default function ProcessSearch({ companyName, accessToken }) {
    ...
    useEffect(() => {
        async function fetchProcesses() {
            ...
            let url = `http://cmsc508.com:8000/~24SP_manoja2/process/${companyName}`;
            if (position) {
            ...
            try {
                ...
                if (position){
                    ...
                    const processes = data[decodedCompanyName][decodeURIComponent(position)].processes;
                    setProcesses(processes);

                }
                else {
                    const data = await response.json();
                    const processes = data[decodedCompanyName].processes;
                    setProcesses(processes);
                }
    ...
    return (
        <div className="flex h-screen w-screen mt-40 justify-between">
            ...
            <div className=" overflow-y-auto p-5 inset-y-0 left-0 w-1/2 mt-8">
                <p className='text-center text-xl mb-6'>Information about the interview process</p>
